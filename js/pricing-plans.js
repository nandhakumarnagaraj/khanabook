...
/* ==========================================================================
   Page-Specific Script for pricing-plans
   ========================================================================== */



// KhanaBook POS — Pricing Page Scripts
let isAnnual = false;
...
window.toggleBillingCycle = function() {
  ...
  if (toggle) {
    ...
    toggle.style.backgroundColor = isAnnual 
      ? 'var(--color-primary)' : 'var(--color-outline-variant)';
  }
  if (thumb) {
    thumb.style.transform = isAnnual ? 'translateX(28px)' : 'translateX(0)';
  }
  if (labelMonthly) {
    labelMonthly.style.color = isAnnual 
      ? 'var(--color-on-surface-variant)' : 'var(--color-on-surface)';
    labelMonthly.style.fontWeight = isAnnual ? '400' : '600';
  }
  if (labelAnnual) {
    labelAnnual.style.fontWeight = isAnnual ? '600' : '400';
    labelAnnual.style.color = isAnnual 
      ? 'var(--color-on-surface)' : 'var(--color-on-surface-variant)';
  }

  // Update prices: find all [data-monthly] elements and swap values
